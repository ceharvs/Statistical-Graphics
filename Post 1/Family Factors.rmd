...
Article  Brigid Schulte, 2013.  
         "Recession had further polarized families"
         Washington Post, Sept. 11, 2013.
           
Document  Qian, Zhenchao. Sept. 2013
         "Divergent Paths of American Families" 
          Department of Sociology at Ohio State University 
          The class folder has a PDF file from the web.  
          Newpapers article focused on Table 7 and
          combined some family types
          
```
The plot represents 24 percents.  We can thinks of these
as forming a 2-way table indexed by race with four categorical values 
and family type with 6 categorical values.  In this class
we usually refer to such categorical variables as factors
there values as level of the factor.  An example of a family type is
...
For each race the Post graph design use 6 "bubbles" arranged
in a circle to show the family type percents.  Their design
shows the 24 values in the bubbles. This serves as both 
spread out table of numbers and a plot. While the original
example may have higher entertainment appeal, it is not as
convenient as a table of numbers for reading and comparing numbers.
Our plots will be better than their plot after revealing 
patterns and more direct leveling of values.  

Below We use dot and bar plots that
have higher perceptual accuracy of extraction than bubble areas. 
We rearrange races and family types to simply the appearance
an bring out patterns.  We use more direct labeling than linking the 6
family types names to symbols using 2 shades of 3 colors.   
It is not so easy to keep 6 color and label pairs in mind.  

We go a little further with some designs by showing
reference values. It is often easier to compare several
values to a single reference value than all pairs of
values to each other.

We could take the next step and explicitly show the 
differences from the reference values for example using
arrow.  

We take the controversial step of creating small
perceptual group by connect points with lines to
provide local focus and support make comparison
across distances that invovle eye saccades.  
I conjecture that the simple shape.  See  
the article by Carr and Sun in the folder.   
...
```{r} 
# Compute the Euclidean distance between White and Black
...
```{r fig.width=6.8, fig.height=3.8}
ord <- c(4,1,5,2,6,3)
...
```{r fig.width=6.8, fig.height=3.1}
...
seem easier to me because use this for
...
from above. The script below puts it together differently
...
qplot(x=percent,y=type,facets=. ~ race, group=race,
...
The percent of families with working fathers is 
...
It is harder for me to imagine connecting centers of bar
...
We can use the saved energy to think about
